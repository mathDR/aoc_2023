...
def split_rule(
    workflow: str,
) -> tuple[str, Callable[[RatingMap], list[tuple[RatingMap, str]]]]:
    name, rule_spec = workflow[:-1].split("{")
        ...
        for rating, op, val, target in rules:
            if op == "<" and r[rating][1] >= val:
                splits.append(
                    (
                        {
                            k: (r[k][0], val - 1) if k == rating else v
                            for k, v in r.items()
                        },
                        target,
                    )
                ...
                splits.append(
                    (
                        {
                            k: (val + 1, r[k][1]) if k == rating else v
                            for k, v in r.items()
                        },
                        target,
                    )
...
rules = {
    name: result for name, result in (rule(spec) for spec in rule_spec.splitlines())
}
parts = [
    {
        cat: int(rating)
        for cat, rating in (r.split("=") for r in spec.strip()[1:-1].split(","))
    }
...
split_rules = {
    name: result
    for name, result in (split_rule(spec) for spec in rule_spec.splitlines())
}
...
print(f"Total ratings combos: {total_combos}")
